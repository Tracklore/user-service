...
async def get_learning_goals_by_user(db: AsyncSession, auth_user_id: int, skip: int = 0, limit: int = 100):
    ...
    try:
        ...
        result = await db.execute(
            select(LearningGoal)
            .filter(LearningGoal.auth_user_id == auth_user_id)
            .offset(skip)
            .limit(limit)
        )
        return result.scalars().all()
    except Exception as e:
        raise Exception(f"Error fetching learning goals for user {auth_user_id}: {str(e)}")

async def get_learning_goals_count_by_user(db: AsyncSession, auth_user_id: int) -> int:
    ...
    try:
        ...
        raise Exception(f"Error counting learning goals for user {auth_user_id}: {str(e)}")
...
async def create_user_learning_goal(db: AsyncSession, learning_goal: LearningGoalCreate, auth_user_id: int):
    ...
    try:
        ...
        await db.rollback()
        raise Exception(f"Error creating learning goal for user {auth_user_id}: {str(e)}")
...
async def get_learning_goal(db: AsyncSession, goal_id: int, auth_user_id: int):
    ...
    try:
        ...
        raise Exception(f"Error fetching learning goal {goal_id} for user {auth_user_id}: {str(e)}")
...
async def update_learning_goal(db: AsyncSession, goal_id: int, auth_user_id: int, learning_goal: LearningGoalUpdate):
    ...
    try:
        ...
        await db.rollback()
        raise Exception(f"Error updating learning goal {goal_id} for user {auth_user_id}: {str(e)}")
...
async def delete_learning_goal(db: AsyncSession, goal_id: int, auth_user_id: int):
    ...
    try:
        ...
        await db.rollback()
        raise Exception(f"Error deleting learning goal {goal_id} for user {auth_user_id}: {str(e)}")