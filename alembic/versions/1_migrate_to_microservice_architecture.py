...
def downgrade() -> None:
    ...
    op.add_column('badges', sa.Column('owner_id', sa.Integer(), nullable=True))
    op.create_foreign_key('badges_owner_id_fkey', 'badges', 'users', ['owner_id'], ['id'])
    op.drop_constraint('fk_badges_auth_user_id', 'badges', type_='foreignkey')
    op.drop_column('badges', 'auth_user_id')
    
    op.add_column('learning_goals', sa.Column('owner_id', sa.Integer(), nullable=True))
    op.create_foreign_key('learning_goals_owner_id_fkey', 'learning_goals', 'users', ['owner_id'], ['id'])
    op.drop_constraint('fk_learning_goals_auth_user_id', 'learning_goals', type_='foreignkey')
